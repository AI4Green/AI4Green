...
def test_check_reaction_for_controlled_substances(app: Flask, client: FlaskClient):
    ...
    with app.app_context():
        with client:
            login(client)
            reaction = services.reaction.get_from_name_and_workbook_id(reaction_name, workbook_id)
            checks = services.controlled_substances.check_reaction_for_controlled_substances(reaction)
            ...
            duplicate_checks = services.controlled_substances.check_reaction_for_controlled_substances(reaction)
...
def create_controlled_substance_reaction(app: Flask) -> Tuple[str, str]:
    ...
    with app.app_context():
        ...
        workbook = services.workbook.get_workbook_from_group_book_name_combination("Test-Workgroup", "Test-Workbook")
        ...
        products=["[O-][N+](N1C2N([N+]([O-])=O)C3N([N+]([O-])=O)C(N(C2N4[N+]([O-])=O)[N+]([O-])=O)C4N([N+]([O-])=O)C13)=O"], # novel compound
        reagents=[""],
        solvent=[-5],
