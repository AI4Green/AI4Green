...
class DataExport:
    ...
    def _make_pdf_export(self):
        ...
        for reaction in self.data_export_request.reactions:
            # save all files to the container
            pdf_file = services.data_export.template_generation.PdfExport(
                reaction, reaction.reaction_id, self.data_export_request.uuid
            )
            pdf_file.make()
            pdf_file.save()
            ...
            #     self._copy_autogenerated_pdf(autogenerated_pdf_blob_name, reaction, blob_service_client)
            # else:
            #     self._make_pdf(reaction, blob_service_client)
        self._make_zip()

    def _make_pdf(
        self, reaction: models.Reaction, blob_service_client: BlobServiceClient
    ):
        pdf = services.data_export.template_generation.render_pdf_template(reaction)
    ...
    def _copy_autogenerated_pdf(
        self,
        autogenerated_pdf_blob_name: str,
