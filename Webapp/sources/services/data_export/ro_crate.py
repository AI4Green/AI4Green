...
class DataExport:
    """
    Base class for functionality that is common across all export modes
    """

    def __init__(self, workgroup_name: str, workbook_name: str):
    ...
    def save_request_to_database(self):
        pass

    def notify_principal_investigators(self):
        pass


class ELNFile(DataExport):
    def __init__(self, workgroup_name: str, workbook_name: str):
        super().__init__(workgroup_name, workbook_name)
        self.root = (
            workgroup_name
            + "-"
            + workbook_name
...
def describe_ro_crate_metadata_json() -> Dict:
    """
...
def reaction_folder(workgroup_name: str, workbook_name: str, reaction: models.Reaction):
...

    {
        ...
        "url": f"{request.base_url}/{reaction.workgroup}/{reaction.workbook}/{reaction.reaction_id}/no",  # may remove no in future
    }
