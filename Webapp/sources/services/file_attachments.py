...
class UploadExperimentDataFiles:
    def __init__(self, ajax_request, autogenerated_file=False):
        ...
        self.form = ajax_request.form
        self.institution = db.session.query(models.Institution).first()
        self.workgroup = ajax_request.form.get("workgroup", None)
        self.workbook = ajax_request.form.get("workbook", None)
        self.reaction_id = ajax_request.form.get("reactionID", None)
        self.validated_files = []
        self.container_client = None
        if (
            self.reaction_id
        ):  # fix for now, maybe can be update to include workgroups ect
    ...
    def save_file_details_to_database(self, file, filename):
        save_dict = self.get_save_info(file, filename)

        models.ReactionDataFile.create(
            reaction=self.reaction.id,
            storage_name=save_dict["storage_name"],
            container_name=save_dict["container_name"],
            uuid=filename,
            display_name=file.filename,
            time_of_upload=save_dict["current_time"],
            file_details=save_dict["file_details"],
    ...
    def process_react_submission(self):
        # keys for files sent from react
        upload_sets = [
            ("newFiles", "newFileFieldResponses"),
            ("files", "fileFieldResponses"),
        ]

        for file_key, meta_key in upload_sets:
            files = self.files_to_upload.getlist(file_key)
            metadata = json.loads(self.form.get(meta_key, "[]"))

            for file, idx in zip(files, metadata):
                if self.file_security_validation(file) == "success":
                    filename = str(uuid.uuid4())
                    self.save_blob(file, filename)
                    self.get_save_info(file, filename)
                    self.save_as_field_response(idx.get("id"))

    def save_as_field_response(self, field_id):
        template_instance_id = self.form.get("recordId")

        # feels like this could be put into a function
        ...
        field_response_value = models.FieldResponseValue.create(
            value=json.dumps(self.save_dict),
            field_response_id=field_response.id,
            time_of_response=datetime.now(),
        )

        return field_response_value

    def get_save_info(self, file, filename):
