...
from sources import db  # imports the User class from sources/models.py
...
class RegistrationForm(FlaskForm):  # this class defines the registration form fields
    username = StringField('Username', validators=[DataRequired(), Length(min=4, max=32), Regexp("^[a-zA-Z0-9_]+$")], render_kw={"placeholder": "Username",
                                                                               "class": "form-control form-control-lg"})
    email = StringField('Email', validators=[DataRequired(), Email(), Length(max=320)], render_kw={"placeholder": "Email",
                                                                                  "class": "form-control form-control-lg"})
    fullname = StringField('Full Name', validators=[DataRequired(), Length(max=50)], render_kw={"placeholder": "Full Name",
                                                                                "class": "form-control form-control-lg"})
    ...
    privacy = BooleanField('I have read and agreed to the conditions in the privacy notice', render_kw={'onclick': 'enableSubmit()'})
    hazard_disclaimer = BooleanField('I have read and understood the hazard disclaimer', render_kw={'onclick': 'enableSubmit()'})
    submit = SubmitField('Register', render_kw={"class": "btn btn-primary", "disabled": "true"})
    ...
    def validate_username(self, username):
        user = select(u for u in db.User if username.data.lower() == u.username.lower()).first()
        if user is not None:
            raise ValidationError('Please use a different username.')

    def validate_email(self, email):
        user = select(u for u in db.User if email.data.lower() == u.email.lower()).first()
        if user is not None:
            raise ValidationError('Please use a different email address.')

    def validate_privacy(self, privacy):
        if privacy.data is not True:
            raise ValidationError('You need to agree to the privacy notice in order to registration')

    def validate_hazard_disclaimer(self, hazard_disclaimer):
        if hazard_disclaimer.data is not True:
            raise ValidationError('You need to agree to the hazard disclaimer in order to registration')
