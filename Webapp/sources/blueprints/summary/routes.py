...
def get_file_attachment_list():
    services.auth.reaction(
        permission_level="view", request_method="POST", async_type="fetch"
    )
    reaction = services.reaction.get_current_from_request(async_type="fetch")
    file_attachments = sorted(
        reaction.file_attachments, key=lambda x: not x.autogenerated
    )
    file_attachments_dict_list = [
        {"name": x.display_name, "uuid": x.uuid} for x in file_attachments
    ]
