async function reactionTableReload() {
  ...
  var limiting_name =
    ...
    js_reaction_table_data["limiting_reactant_table_number"];
  ...
  const solventFields = {
    volumes: "-volume",
    concentrations: "-rounded-concentration",
    names: "",
    hazards: "-hazards",
    physical_forms: "-physical-form",
  };
  ...
  // reload units
  const unitFields = Object.assign({
    mass_units: "js-mass-unit",
    amount_units: "js-amount-unit",
    product_amount_units: "js-product-amount-unit",
    solvent_volume_units: "js-solvent-volume-unit",
    volume_units: "js-volume-unit",
    product_mass_units: "js-product-mass-unit",
  });
  ...
  function checkStyleElements(fieldID) {
    ...
    if (fieldID.includes("reactant") || fieldID.includes("reagents")) {
      let limitingReactantTableNumber =
        js_reaction_table_data["limiting_reactant_table_number"];
      if (
        fieldID.slice(-1) === limitingReactantTableNumber &&
        fieldID.includes("equivalent") &&
        fieldID.includes("reactant")
      ) {
        return;
      }
      if (
        fieldID.slice(-1) !== limitingReactantTableNumber &&
        fieldID.includes("rounded-mass")
      ) {
        return;
      }
    } else {
