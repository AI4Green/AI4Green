...
$(function() {
    ...
    applyMassCellStylingOnChange()
});

/**
 * Applies red highlight styling to the unreacted, and yield cells if null value.
 * Only monitors after the user edits the cell or tries to lock the reaction.
 * 0 is excluded because unreacted mass/yield could feasibly have values of 0
 */
function applyMassCellStylingOnChange() {
    autoChangeRequiredStyling("#js-unreacted-reactant-mass", [0, "0"])
    autoChangeRequiredStyling("#js-real-product-mass", [0, "0"])
}
