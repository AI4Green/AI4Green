...
async function printAndSendPDF() {
  ...
  // Get the content of the printed window
  const printedContent = document.documentElement.outerHTML;

  // Convert the content to PDF
  const pdfBlob = await htmlToPDF(printedContent);

  // Send the PDF to the backend
  const formData = new FormData();
  formData.append("pdfFile", pdfBlob, "printed-document.pdf");
