...
import pytz
from sources.auxiliary import sanitise_user_input, get_data, get_smiles
...
def new_reaction():
    ...
    workgroup_selected_obj = select(x for x in db.WorkGroup if x.name == workgroup).first()
    workbook_object = select(
        wb for wb in db.WorkBook if wb.name == workbook and wb.group == workgroup_selected_obj).first()
    reaction_name = sanitise_user_input(request.form['reactionName'])
    reaction_id = request.form['reactionID']
    name_check = check_reaction()
    creator = select(x for x in db.Person if x.user.email == current_user.email).first()
    ...
    reaction_id_check = select(
        rxn for rxn in db.Reaction if rxn.reaction_id == reaction_id and workbook_object == rxn.workbooks).first()
    ...
    if b'This reaction name is unique' in name_check.data:
        ...
        reaction_table = json.dumps({'amount_units': 'mmol', 'mass_units': 'mg', 'volume_units': 'mL',
                                     'solvent_volume_units': 'mL', 'product_amount_units': 'mmol',
                                     'product_mass_units': 'mg',
                                     'reactant_masses': [], 'reactant_masses_raw': [],
                                     'reactant_amounts': [], 'reactant_amounts_raw': [],
                                     'reactant_volumes': [], 'reactant_volumes_raw': [],
                                     'reactant_equivalents': [], 'reactant_physical_forms': [],
                                     'reactant_densities': [], 'reactant_concentrations': [],
                                     'reagent_names': [], 'reagent_molecular_weights': [],
                                     'reagent_densities': [], 'reagent_concentrations': [],
                                     'reagent_amounts': [], 'reagent_amounts_raw': [],
                                     'reagent_equivalents': [],
                                     'reagent_physical_forms': [], 'reagent_hazards': [],
                                     'reagent_masses': [], 'reagent_masses_raw': [],
                                     'reagent_volumes': [], 'reagent_volumes_raw': [], 'solvent_volumes': [],
                                     'solvent_names': [], 'solvent_concentrations': [],
                                     'solvent_hazards': [], 'solvent_physical_forms': [],
                                     'product_amounts': [], 'product_amounts_raw': [],
                                     'product_masses': [], 'product_masses_raw': [], 'product_physical_forms': []
                                     })

        summary_table = json.dumps({'real_product_mass': "", 'unreacted_reactant_mass': "",
                                    'reaction_temperature': "", 'batch_flow': "-select-",
                                    'element_sustainability': 'undefined', 'isolation_method': "undefined",
                                    'catalyst_used': "-select-", 'catalyst_recovered': "-select-",
                                    'custom_protocol1': "", 'custom_protocol2': "",
                                    'other_hazards_text': "",
                                    'researcher': "", 'supervisor': "", 'radio_buttons': []})

        # add reaction to database
        db.Reaction(creator=creator, reaction_id=reaction_id, time_of_creation=current_time, name=reaction_name,
                    workbooks=workbook_object, status="active", complete="not complete",
                    reaction_table_data=reaction_table, summary_table_data=summary_table)
        # load sketcher
...
def autosave():
    ...
    reaction_table = json.dumps({
        # reaction data
        'reaction_smiles': reaction_smiles, 'reaction_name': reaction_name,
        'reaction_description': reaction_description,
        # reactant data
        'reactant_smiles': reactant_smiles_ls, 'reactant_masses': reactant_masses,
        'reactant_amounts': reactant_amounts, 'reactant_amounts_raw': reactant_amounts_raw,
                                 'reactant_volumes': reactant_volumes, 'reactant_volumes_raw': reactant_volumes_raw,
                                 'reactant_equivalents': reactant_equivalents,
                                 'reactant_physical_forms': reactant_physical_forms,
                                 'reactant_masses_raw': reactant_masses_raw, 'reactant_densities': reactant_densities,
                                 'reactant_concentrations': reactant_concentrations,
                                 'limiting_reactant_table_number': limiting_reactant,
                                 'reagent_smiles': reagent_smiles_ls, 'reagent_names': reagent_names,
                                 ...
                                 'reagent_amounts': reagent_amounts, 'reagent_amounts_raw': reagent_amounts_raw,
                                 'reagent_equivalents': reagent_equivalents,
                                 'reagent_physical_forms': reagent_physical_forms, 'reagent_hazards': reagent_hazards,
                                 'reagent_masses': reagent_masses, 'reagent_masses_raw': reagent_masses_raw,
                                 'reagent_volumes': reagent_volumes, 'reagent_volumes_raw': reagent_volumes_raw,
                                 'solvent_ids': solvent_primary_keys_ls, 'solvent_volumes': solvent_volumes,
                                 'solvent_names': solvent_names, 'solvent_concentrations': solvent_concentrations,
                                 'solvent_hazards': solvent_hazards, 'solvent_physical_forms': solvent_physical_forms,
                                 'product_amounts': product_amounts, 'product_amounts_raw': product_amounts_raw,
                                 'product_masses': product_masses,
                                 'product_masses_raw': product_masses_raw, 'product_smiles': product_smiles_ls,
                                 'product_physical_forms': product_physical_form, 'main_product': main_product,
                                 'amount_units': amount_units, 'mass_units': mass_units, 'volume_units': volume_units,
                                 ...
                                 'product_names': product_names, 'product_molecular_weights': product_molecular_weights,
    ...
    summary_table = json.dumps(
        {'real_product_mass': real_product_mass, 'unreacted_reactant_mass': unreacted_reactant_mass,
         'reaction_temperature': reaction_temperature,
         'element_sustainability': element_sustainability, 'batch_flow': batch_flow,
         'isolation_method': isolation_method,
         'catalyst_used': catalyst_used, 'catalyst_recovered': catalyst_recovered,
         'custom_protocol1': custom_protocol1, 'custom_protocol2': custom_protocol2,
         'other_hazards_text': other_hazards_text,
         'researcher': researcher, 'supervisor': supervisor, 'radio_buttons': radio_buttons,
         'summary_to_print': summary_to_print, 'mass_efficiency': mass_efficiency, 'to_export': to_export})
    reaction = select(
        x for x in db.Reaction if x.reaction_id == reaction_id and x.workbooks.name == workbook_name and
        x.workbooks.group.name == workgroup).first()
    ...
    if complete == "complete":
        missing_data_fields = [reactant_masses, reactant_equivalents, reagent_equivalents, reagent_names, solvent_names,
                               solvent_volumes, reagent_physical_forms, solvent_physical_forms, product_physical_form]
        missing_data = False
        for x in missing_data_fields:
            if '' in x or '0' in x:
                missing_data = True
        if unreacted_reactant_mass == "" or real_product_mass == "":
            return jsonify({'feedback': 'Please enter unreacted and product mass to mark reaction as complete!'})
        elif missing_data:
            return jsonify({'feedback': 'Please fill in all required data to mark reaction as complete!'})
        else:
            feedback = "Reaction locked"
    # update the database entry for the reaction
    update_dict = {'time_of_update': current_time, 'name': reaction_name, 'complete': complete,
                   'reaction_smiles': reaction_smiles,
                   'description': reaction_description, 'reactants': reactant_smiles_ls,
                   'products': product_smiles_ls,
                   'reagents': reagent_smiles_ls, 'solvent': solvent_primary_keys_ls, 'workbooks': workbook,
                   'reaction_table_data': reaction_table, 'summary_table_data': summary_table}
    reaction.set(**update_dict)
    return jsonify({'feedback': feedback})


@save_reaction_bp.route('/_autosave_sketcher', methods=['POST'])
@login_required
def autosave_sketcher():
    current_time = datetime.now(pytz.timezone('Europe/London')).replace(tzinfo=None)
    reaction_smiles = str(request.form['reactionSmiles'])
    reaction_id = str(request.form['reactionID'])
    # Takes the first workbook the user has access to
...
def check_reaction():
    ...
    reaction_name_check = select(
        c for c in db.Reaction if
        c.name.lower() == reaction_name.lower() and c.workbooks.name == workbook_name and c.workbooks.group.name == workgroup_name).first()
